  ...
  const fetchAndRenderSales = async (startDate, endDate) => {
    ...
    try {
      ...
      const [ordersSnapshot,
        analyticsSnapshot] = await Promise.all([
          getDocs(ordersQuery),
      ...
      totalValueEl.textContent = totalValue.toLocaleString('pt-BR', {
        style: 'currency', currency: 'BRL'
      });
      totalCountEl.textContent = totalCount;
      if (totalDeliveryFeesEl) {
        totalDeliveryFeesEl.textContent = totalDeliveryFees.toLocaleString('pt-BR', {
          style: 'currency', currency: 'BRL'
        });
      }
      ...
      if (paymentSummaryContainer) {
        ...
        salesData.forEach(order => {
          ...
          if (method in paymentTotals) {
            paymentTotals[method] += value;
          }
        });
        paymentSummaryContainer.innerHTML = `
        <div class="payment-summary-box pix"><span class="label"><i class="fas fa-qrcode"></i> Pix</span><span class="value">${paymentTotals['Pix'].toLocaleString('pt-BR',
          {
            style: 'currency',
            currency: 'BRL'
          })}</span></div>
        <div class="payment-summary-box cartao"><span class="label"><i class="fas fa-credit-card"></i> Cartão</span><span class="value">${(paymentTotals['Cartão de Crédito'] + paymentTotals['Cartão de Débito']).toLocaleString('pt-BR',
          {
            style: 'currency',
            currency: 'BRL'
          })}</span></div>
        <div class="payment-summary-box dinheiro"><span class="label"><i class="fas fa-money-bill-wave"></i> Dinheiro</span><span class="value">${paymentTotals['Dinheiro'].toLocaleString('pt-BR',
          {
            style: 'currency',
            currency: 'BRL'
          })}</span></div>
      ...
      if (bestsellersContainer && bestsellersList && salesData.length > 0) {
        ...
        const sortedProducts = Object.entries(productCount).sort((a,
          b) => b[1] - a[1]);
        bestsellersList.innerHTML = sortedProducts.slice(0, 5).map(([name,
          quantity],
          index) => {
          const rank = index + 1;
