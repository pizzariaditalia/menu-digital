...
// --- FUNÇÃO DE AJUDA ---
const formatPriceAdmin = (price) => (typeof price === 'number') ? price.toLocaleString('pt-BR', {
...
async function initializeCardapioSection() {
  ...
  function loadItemsIntoCategory(categoryKey,
    container) {
    container.innerHTML = '';
    ...
    categoryListContainer.querySelectorAll('.category-item').forEach(header => {
      header.addEventListener('click', (e) => {
        ...
        if (e.target.closest('.add-item-btn')) {
          ...
          if (expandBtnIcon) {
            expandBtnIcon.classList.toggle('fa-chevron-down', isVisible); expandBtnIcon.classList.toggle('fa-chevron-up', !isVisible);
          }
          if (!isVisible) {
  ...
  function renderRecipeList() {
    ...
    recipeListContainer.querySelectorAll('.remove-ingredient-btn').forEach(button => {
      button.addEventListener('click', () => {
        currentItemRecipe.splice(button.dataset.index, 1); renderRecipeList();
      });
    });
    updateTotalItemCost();
  }
  ...
  function updateTotalItemCost() {
    const totalCost = calculateItemCost( {
  ...
  if (itemEditorForm) {
    itemEditorForm.addEventListener('submit', async (e) => {
      ...
      if (isNaN(updatedItemData.price)) {
        window.showToast("Por favor, insira um preço válido.", "error"); return;
      }
      if (isNewItem) {
