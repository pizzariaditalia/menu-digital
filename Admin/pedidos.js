// pedidos.js - VERSÃO COM INICIALIZAÇÃO CORRIGIDA
...
function listenForRealTimeOrders() {
    if (unsubscribeFromOrders) {
        console.log("Listener de pedidos já está ativo.");
        return;
    };
    ...
    console.log("Configurando listener de pedidos em tempo real...");
    unsubscribeFromOrders = onSnapshot(q, snapshot => {
        console.log("Novos dados de pedidos recebidos.");
...
// --- ALTERAÇÃO PRINCIPAL ---
// A função de inicialização agora só adiciona os listeners e busca os entregadores.
// A busca de pedidos em tempo real fica separada para ser chamada no momento certo.
async function initializeOrdersSection() {
    if (ordersSectionInitialized) {
        // Se já foi inicializado, apenas garante que os pedidos estão sendo renderizados.
        renderOrders(); 
        return;
    }
    ordersSectionInitialized = true;
    
    console.log("Módulo Pedidos.js: Configurando listeners pela primeira vez...");
    
    // Configura os listeners dos botões e filtros
    ...
    // Busca os entregadores e ativa o listener de pedidos em tempo real.
