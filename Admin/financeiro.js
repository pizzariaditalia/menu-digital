...
async function calculateAndUpdateVariableCost() {
  ...
  try {
    ...
    const [ordersSnapshot,
      ingredientsSnapshot] = await Promise.all([getDocs(ordersQuery), getDocs(ingredientsQuery)]);
...
function setupProjections() {
  ...
  const inputs = [custosFixosInput,
    proLaboreInput,
    custoIngredientesInput,
    outrosCustosInput];
...
async function initializeFinanceiroSection() {
  ...
  async function fetchFinancialData(startDate, endDate) {
    ...
    const [ordersSnapshot,
      lancamentosSnapshot] = await Promise.all([getDocs(ordersQuery), getDocs(lancamentosQuery)]);
    return {
      pedidos: ordersSnapshot.docs.map(doc => ({
        id: doc.id, ...doc.data()
      })),
  ...
  function processAndRenderData( {
    ...
    renderSummaryDashboard( {
      faturamentoBruto, despesasTotais, lucroLiquido, ticketMedio
    });
  ...
  lancamentoForm.addEventListener('submit',
    async (e) => {
      e.preventDefault();
      const {
        collection,
