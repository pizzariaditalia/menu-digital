...
// DENTRO DE notifications.js (FUNÇÃO getAndSaveToken CORRIGIDA)
...
async function getAndSaveToken() {
    ...
    const { doc, updateDoc, arrayUnion } = window.firebaseFirestore; // Adicionei o arrayUnion aqui
    const db = window.db;

    try {
        const vapidKey = 'BEu5mwSdY7ci-Tl8lUJcrq12Ct1w62_2ywucGfPq0FanERTxEUk7wB9PK37dxxles-9jpbN2nsrv3S2xnzelqYU';
        
        // CORREÇÃO: Pegamos a referência do nosso Service Worker principal ('sw.js')
        const swRegistration = await navigator.serviceWorker.ready;
        const fcmToken = await getToken(messagingInstance, { 
            vapidKey: vapidKey,
            serviceWorkerRegistration: swRegistration  // E informamos ao Firebase qual SW usar
        });
        ...
        if (fcmToken) {
            ...
            await updateDoc(customerDocRef, {
                notificationTokens: arrayUnion(fcmToken) // Usando arrayUnion para adicionar sem duplicar
            });
