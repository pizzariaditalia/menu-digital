// menu.js - VERSÃO FINAL COMPLETA (COM CARROSSEL, PROMOÇÕES E CATEGORIAS DINÂMICAS)
...
function initializeSiteLogic() {
    ...
    function findItemAcrossCategories(itemId) {
        for (const categoryKey in window.menuData) {
            ...
            if (category && Array.isArray(category.items)) {
                ...
                if (foundItem) return { ...foundItem, category: categoryKey };
            }
        }
        return null;
    }
    ...
    if (minOrderLine && minOrderValueSpan && window.appSettings?.storeInfo?.minOrderValue > 0) {
        minOrderValueSpan.textContent = formatPrice(window.appSettings.storeInfo.minOrderValue);
    ...
    function createMenuItemHTML(item) {
        ...
        let descHTML = item.description ? `<p class="item-description">${item.description}</p>` : '';
        const imagePath = (item.image || 'img/placeholder.png').replace('../', '');
        return `<div class="menu-item" data-item-id="${item.id}" data-category="${item.category || ''}"><img src="${imagePath}" alt="${item.name}" class="item-image" onerror="this.onerror=null;this.src='img/placeholder.png';"><div class="item-details"><h4>${item.name}</h4>${descHTML}${priceHTML}</div><button class="add-to-cart-button" data-item-id="${item.id}" data-category="${item.category || ''}">+</button></div>`;
    };
    ...
    function renderCategory(categoryName) {
        ...
        if (visibleItems.length > 0) {
            contentDiv.innerHTML = visibleItems.map(item => createMenuItemHTML({ ...item, category: categoryName })).join('');
        } else {
